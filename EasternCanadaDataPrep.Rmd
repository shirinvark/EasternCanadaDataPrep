...
---
Executive summary
EasternCanadaDataPrep is a SpaDES module that constructs a raster-based
landbase accounting framework for forest modelling applications in Eastern Canada.

The module defines where harvesting is spatially permissible by applying
legal and physical constraints to a wall-to-wall planning raster.
It does not perform forest classification, yield modelling, or harvest allocation.

This module is intentionally limited to spatial accounting only and produces
inputs required by downstream modules such as LandR, AAC calculation,
...
All ecological interpretation and management decisions are deferred to
downstream modules.
...
“Harvestable” means not legally excluded, not “forested”, “merchantable”,
or “operable”.
...
land-cover data. If required, land cover must be supplied by an upstream module.
...
Land cover is not required for constructing the legal harvestable mask
in this module.
...
They are rasterized to the planning grid using a center-of-pixel rule to
ensure consistent assignment of cells to management units.

Protected and conserved areas (CPCAD)
Protected and conserved areas are derived from the
...
Hydrology is represented as a fractional raster of riparian influence,
not as explicit vector hydrological features.
...
This table enables transparent and reproducible area calculations without
repeated raster processing.
...
It provides spatial constraints only and makes no assumptions about forest
condition or management objectives.
